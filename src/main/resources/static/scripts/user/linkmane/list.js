layui.use(['form', 'layer', 'table', 'laytpl', 'laydate'], function () {
    ...
    var tableIns = table.render({
        elem: '#List',
        url: web.rootPath() + 'linkmane/list',
        cellMinWidth: 95,
        page: true,
        ...
        cols: [[
            {type: "checkbox", fixed: "left", width: 50},
                    ...
                    {field: 'custName', title: '所属企业', minWidth: 130, align: "center"},
    ...
    var $ = layui.$, active = {
        reload: function () {
            ...
            var selectedCust = $("#searchForm").find("select[name='selectedCust']").val().trim();
            //表格重载
            tableIns.reload({
        ...
        switch (obj.event) {
            case 'update':
                layer.open({
                    ...
                    fixed: false,
                    maxmin: true,
                    content: web.rootPath() + "linkmane/" + data.id + ".html?_"
                });
                break;
            case 'delete':
                var index = layer.confirm('确定要删除?', {
                    ...
                    $.ajax({
                        url: web.rootPath() + "linkmane/delete/" + data.id,
                        type: "delete",
